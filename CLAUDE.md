...
_Last updated: 2026-08-17 (Session 4.1 built · PR pending)_
...
- **Two OpenEMR-side items block full write verification** (code ready, gracefully degrades): (1) swap `OPENEMR_CLIENT_ID/SECRET` env values to the new "GFC Care Platform (server)" client, (2) give the `gfc-app-api` user a clinician-grade ACL group (Administration → Users) — encounter writes, document uploads, and practitioner/org reads 403 until then. Re-verify H&P write + PDF filing after.
...
**08/2026 — Session 4.1 built (branch `claude/clinician-care-plan-auth-ra1t7k`, PR pending).** Clinician workspace + OpenEMR write per the Session 4.1 prompt. Locked in:
...
- **OpenEMR items still open (Session 0 / §15):** swap env `OPENEMR_CLIENT_ID/SECRET` to the new client; `gfc-app-api` needs a clinician-grade ACL group (encounter writes, document uploads, Practitioner/Organization/Coverage reads all 403 on user ACL, not scopes); practice/facility + provider records unverifiable via API until that ACL fix; fee schedule / Availity / ERA config remain manual checks (B-series). Preflight artifacts labeled "(TEST DATA)" were left on dev patient TEST PatientOne.
