...
- **Post-merge follow-up (owner-directed, 2026-08-30): the Portal Hub is SUPER-ADMIN ONLY.** Bianca's walkthrough found a clinical-only user landing on an empty "No Portals Available" hub. Now every non-admin is routed straight to their destination on login (`login.html directDestination()`: client/family → care portal, `hasClinicalAccess` → `/clinical`, manager/`hasAdminHubAccess` → `/admin`, `hasClientPortalAdminAccess` → `/portal`, vendor/`hasServicePortalAccess` → `/service-portal`; multiple flags = that priority order). Only Super Admins see the hub, which also gained a Clinical Workspace tile. A user with zero access flags still sees the hub's empty-state card (nowhere to send them).
