...
_Last updated: 2026-08-16 (Session 3.3 merged, PR #16)_
...
- **Messages:** basic client→admin send persists (`POST /api/gfc/messages`, channel `admin`, 4000-char cap, family blocked). Staff-side inbox + full channel matrix stay in Session 9; until then admin-bound messages live in the `gfc_messages` store (interim admin inbox **deferred to Session 9** per Bianca, 08/16 — no pull-forward).
