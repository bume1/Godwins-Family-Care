...
async function main() {
  const db = require(path.join(__dirname, '..', 'dataStore'));
  ...
  // no own-client and is out of scope here.
  const ownClientId = (u) => {
    if (!u) return null;
  ...
  for (const r of misfiled) {
    ...
    console.log(`  [${r.message.sent_at}] ${name(r.senderClientId)} wrote into ${name(r.threadClientId)}'s ` +
                `${(r.thread && r.thread.channel) || m_channel(r)} thread`);
  ...
  // A thread's preview is a copy of its last message, so a quarantined message
  // would otherwise keep showing in the list even once it is gone from the body.
...
const m_channel = (r) => (r.message && r.message.channel) || 'unknown';
