...
> **RESCUED AND RE-POINTED, 2026-09-22.** This spec was written in the Replit era and sat on an
> unmerged branch (`claude/nifty-faraday-b1i84b`, commit `c7a3469`) — CLAUDE.md recorded it as
> "shelved (not built)", which is how it fell off the map. The owner asked for exactly this
> capability on 2026-09-22, so it is back on main.
>
> **§0 below is SUPERSEDED and kept only for the record.** Its posture is inverted now:
> Session 5 merged, the app runs on AWS inside the BAA boundary, and there is no Replit.
> The binding rules are **`GFC_Clinical_Completeness_Spec_v1.md` §10.3**, which is normative —
> Bedrock with an IAM role (never keys), **zero data retention explicitly configured** as a STOP
> condition, propose-never-commit, unverified-until-touched, and one audit entry per model call
> recording accept/edit/discard.
>
> **§6 is also superseded in one respect:** it reasons from Resend having no BAA. Gmail on Google
> Workspace is live and BAA-covered (2026-09-13), so a notice may now carry detail where the
> transport is covered. The rule was always about COVERAGE, never about a vendor — `notifications.js`
> re-reads it on every send. Keeping the nudge link-only is still the right default.
>
> Everything else here — the ledger, the provenance precedence, the version override, the worked
> examples — is the design being built and is unchanged.

## 0. Compliance posture (SUPERSEDED — see the banner above)
