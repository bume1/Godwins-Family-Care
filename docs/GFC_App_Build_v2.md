...
- `GFC_Clinical_Completeness_Spec_v1.md` — OpenEMR clinician-capability coverage for the clinical line: note→billing route (§2), sign-and-close (§3), attribution (§4), P0/P1/P2 build order (§6), ICD-10/CPT source of truth (§7), coding-assist tiers (§8). **Session 4.4 spec.**
...
- **Clinical completeness (Session 4.4, the set that makes a visit billable):** ICD-10-coded diagnoses and CPT/HCPCS service capture, prescription recording, lab/imaging **order capture**, encounter **sign-and-close** (attestation; closed notes read-only, corrections as addenda), and the **note→billing route** — the charge written to OpenEMR's fee sheet for that encounter, rendering provider = signing clinician, billing provider = `gfc_payer_credentialing.billing_npi_used`. **OpenEMR is the source of truth for both code sets; the app never keeps its own list.** Deferred by owner decision: Quest HL7 lab interface and e-prescribing transmission — both are recorded in the chart, transmitted manually. Full detail, priorities and coding-assist tiers: `GFC_Clinical_Completeness_Spec_v1.md`.
