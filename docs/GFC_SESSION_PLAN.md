...
## Rough timeline
- **4.5 Native 8.4 writes + Phase 6B routes — ⛔ BLOCKED at preflight (2026-09-06).** The session stopped at preflight item 1 and wrote no native-write or 6B code: the deployed `OPENEMR_CLIENT_ID` is still the **August v2 appointment client** (`dlHHtxPDc1gS…`), so the token carries **42 scopes, not 54** — none of `user/billing.*`, `user/order.*`, `user/codes.read`, `user/prescription.*`. Every 6B route and `/api/prescription` refused at the **scope** layer (`Unauthorized`), not the ACL. The server is healthy: 8.4.0, database 543, and the 6B patch is installed (an unauthenticated `GET /api/codes` returns 401, not 404). **Two owner actions unblock it, and both are required:** swap `OPENEMR_CLIENT_ID`/`OPENEMR_CLIENT_SECRET` to the v4 client and restart, and set the billing NPI (sign-and-close is blocked without it, so the charge write cannot be proven end to end). The app-side half of the fix is merged in this branch — the scope list now requests all 54, since a token carries only the intersection of requested and registered. Preflight also exposed and fixed three app defects: the document route now rejects the patient uuid (care-plan PDFs had silently stopped filing into OpenEMR), a 404 on the appointment/medication lists means "empty" not "broken" (a red error on every newly linked patient's Appointments tab), and a narrowed token was indistinguishable from a healthy one in the UI (which is how the stale client survived the upgrade). Full record: `OPENEMR_SERVER_DEFECTS_2026-08.md`. Prompt: `GFC_Session4.5_ClaudeCode_Prompt.md`.
